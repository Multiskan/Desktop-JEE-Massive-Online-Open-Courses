...
public class List_send_Message extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					List_send_Message frame = new List_send_Message();
	...
	//	panel.setBounds(0, 0, 494, 33);
		...
		String[] filterTypeStrings = { "    id    ", "    Subject    ", "    body    ", "    status    ", "    date    " };
...
//			    };
	...
	public void loadTableData(){
		model = new DefaultTableModel(
				...
				new String[] {
				"Id", "Subject", "body",  "state","date"
			}
		);

			//supposant que id user est 3
			List<Message>messages=GestMessageBD.getAllsendMessage(1);
		
			for(Object app:messages){
				Message appr=(Message)app;
				System.out.println(appr.getSubject());
			
				model.addRow(new Object[]{appr.getId_Message(),appr.getSubject(),appr.getBody(),appr.getState_Message(),appr.getDate_Message()});
